...
interface MartingaleConfig { enabled:boolean; maxStep:number; multiplier:number; }
...
const SL: React.FC<{children:React.ReactNode;accent?:string}> = ({children,accent}) => (
  <div style={{display:'flex',alignItems:'center',gap:8,marginBottom:10,marginTop:4}}>
    ...
    <div style={{flex:1,height:1,background:'linear-gradient(to right,rgba(52,211,153,0.18),transparent)'}}/>
  </div>
);
...
const Toggle: React.FC<{checked:boolean;onChange:(v:boolean)=>void;disabled?:boolean}> = ({checked,onChange,disabled}) => (
  <label style={{display:'inline-flex',alignItems:'center',cursor:disabled?'not-allowed':'pointer',opacity:disabled?0.4:1}}>
    ...
    <div style={{width:44,height:22,borderRadius:22,position:'relative',transition:'all 0.2s',background:checked?'rgba(52,211,153,0.2)':'rgba(255,255,255,0.06)',border:`1px solid ${checked?'rgba(52,211,153,0.5)':'rgba(255,255,255,0.12)'}`}}>
      <div style={{position:'absolute',top:2,width:16,height:16,borderRadius:'50%',transition:'left 0.2s',left:checked?23:2,background:checked?C.cyan:'rgba(255,255,255,0.5)'}}/>
    </div>
  </label>
);
...
const AssetCard: React.FC<{asset?:StockityAsset|null;mode:TradingMode;isLoading?:boolean}> = ({asset,mode,isLoading}) => {
  const modeCol  = mode==='ctc'?C.violet:C.cyan;
...
({status,logs,isLoading,fillHeight}) => {
  ...
  const icon = (status?.mode==='CTC')?<Copy style={{width:14,height:14}}/>:<Zap style={{width:14,height:14}}/>;
    ...
    <Card style={{display:'flex',flexDirection:'column',height:fillHeight?'100%':undefined}}>
      ...
      {isLoading?(
        ...
        <div style={{flex:1,display:'flex',flexDirection:'column',alignItems:'center',justifyContent:'center',padding:20,gap:8}}>
          ...
          <p style={{fontSize:10,color:C.muted,textAlign:'center',lineHeight:1.5}}>Konfigurasi dan mulai sesi di bawah</p>
        </div>
      ):(
        <div style={{flex:1,overflowY:'auto',padding:'12px 14px',display:'flex',flexDirection:'column',gap:10}}>
          ...
          {logs.length>0&&(
            <div style={{borderTop:`1px solid ${C.bdr}`,paddingTop:10}}>
              <p style={{fontSize:9,color:C.muted,marginBottom:6,textTransform:'uppercase',letterSpacing:'0.08em'}}>Riwayat Terakhir</p>
...
const ModeSessionPanel: React.FC<{
  ...
  fillHeight?:boolean;
}> = ({mode,onModeChange,locked,blockedModes,orders,logs,onOpenModal,isRunning,ftStatus,ftLogs,ftLoading,fillHeight}) => {
  const [dropOpen,setDropOpen] = useState(false);
  const MODES = [
    {v:'schedule' as TradingMode,label:'Signal',icon:<Calendar style={{width:12,height:12}}/>,accent:C.cyan,desc:'Order terjadwal'},
    {v:'fastrade' as TradingMode,label:'FastTrade',icon:<Zap style={{width:12,height:12}}/>,accent:C.cyan,desc:'Auto per candle'},
...
const SettingsCard: React.FC<{
  ...
  disabled?:boolean;
}> = ({mode,assets,assetRic,onAssetChange,isDemo,onDemoChange,duration,onDurationChange,amount,onAmountChange,martingale,onMartingaleChange,ftTf,onFtTfChange,stopLoss,onSlChange,stopProfit,onSpChange,disabled}) => {
    ...
    <>
      ...
      <Card style={{opacity:disabled?0.65:1}}>
        <button onClick={()=>setOpen(!open)} style={{width:'100%',display:'flex',alignItems:'center',justifyContent:'space-between',padding:'12px 16px',background:'transparent',border:'none',borderBottom:open?`1px solid ${C.bdr}`:'none',cursor:'pointer'}}>
          ...
          <div style={{display:'flex',alignItems:'center',gap:8}}>
            <span style={{fontSize:10,padding:'2px 8px',borderRadius:99,background:`${ac}12`,color:ac,border:`1px solid ${ac}25`}}>{mode==='ctc'?'CTC':mode==='fastrade'?'FastTrade':'Signal'}</span>
        ...
        {open&&(
          <div style={{padding:'14px 16px',pointerEvents:disabled?'none':undefined}}>
            ...
            <Divider/>
            <SL>Martingale</SL>
            <div style={{padding:12,borderRadius:12,background:'rgba(52,211,153,0.06)',border:'1px solid rgba(52,211,153,0.12)',marginBottom:12}}>
              <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',marginBottom:martingale.enabled?12:0}}>
                ...
                <Toggle checked={martingale.enabled} onChange={v=>set('enabled',v)} disabled={disabled}/>
              </div>
              {martingale.enabled&&(
                <div style={{paddingTop:12,borderTop:`1px solid ${C.bdr}`,display:'flex',flexDirection:'column',gap:10}}>
                  <div>
                    ...
                    <div style={{display:'flex',gap:6}}>
                      {[1,2,3,4,5].map(k=>(
                        <button key={k} type="button" disabled={disabled} onClick={()=>set('maxStep',k)} style={{flex:1,padding:'6px 0',borderRadius:8,fontSize:11,fontWeight:700,cursor:'pointer',background:martingale.maxStep===k?'rgba(52,211,153,0.15)':'rgba(255,255,255,0.04)',border:`1px solid ${martingale.maxStep===k?'rgba(52,211,153,0.5)':'rgba(255,255,255,0.08)'}`,color:martingale.maxStep===k?C.cyan:'rgba(255,255,255,0.5)'}}>K{k}</button>
                      ))}
                    </div>
                  </div>
                  <div>
                    ...
                    <div style={{display:'flex',gap:6}}>
                      {[1.5,2,2.5,3,5].map(m=>(
                        <button key={m} type="button" disabled={disabled} onClick={()=>set('multiplier',m)} style={{flex:1,padding:'6px 0',borderRadius:8,fontSize:11,fontWeight:700,cursor:'pointer',background:martingale.multiplier===m?'rgba(52,211,153,0.15)':'rgba(255,255,255,0.04)',border:`1px solid ${martingale.multiplier===m?'rgba(52,211,153,0.5)':'rgba(255,255,255,0.08)'}`,color:martingale.multiplier===m?C.cyan:'rgba(255,255,255,0.5)'}}>
                          {m}x
                        </button>
                      ))}
                    </div>
...
const ControlCard: React.FC<{
  ...
  const isActive = mode==='schedule' ? (isSchedRunning||isSchedPaused) : isFtRunning;
  const ac = mode==='ctc'?C.violet:C.cyan;

  const si = isSchedRunning&&mode==='schedule' ? {label:'Aktif',col:C.cyan,pulse:true}
    : isSchedPaused&&mode==='schedule' ? {label:'Dijeda',col:'#6ee7b7',pulse:false}
    : isFtRunning&&mode!=='schedule' ? {label:'Aktif',col:ac,pulse:true}
    : {label:'Standby',col:C.muted,pulse:false};

  const icon = mode==='ctc' ? <Copy style={{width:14,height:14}}/> : mode==='fastrade' ? <Zap style={{width:14,height:14}}/> : <Calendar style={{width:14,height:14}}/>;
  const title = mode==='ctc' ? 'CTC Bot' : mode==='fastrade' ? 'FastTrade' : 'Bot Signal';
  const subtitle = mode==='ctc' ? 'Copy the Candle · 1m' : mode==='fastrade' ? 'Auto per candle' : 'Eksekusi terjadwal';
  const pnlPos = profit>=0;

  const wins = ftStatus?.totalWins??0;
  const losses = ftStatus?.totalLosses??0;
  const total = ftStatus?.totalTrades??0;
  const wr = total>0?Math.round((wins/total)*100):null;

  return (
    <Card style={{borderColor:isActive?(mode==='ctc'?'rgba(167,139,250,0.3)':'rgba(52,211,153,0.3)'):undefined}}>
      <button onClick={()=>setOpen(!open)} style={{width:'100%',display:'flex',alignItems:'center',justifyContent:'space-between',padding:'12px 16px',background:'transparent',border:'none',borderBottom:open?'1px solid rgba(255,255,255,0.05)':'none',cursor:'pointer'}}>
        <div style={{display:'flex',alignItems:'center',gap:10}}>
          ...
          <div>
            <span style={{display:'block',fontSize:13,fontWeight:600,lineHeight:1,marginBottom:3,color:C.text}}>{title}</span>
      ...
      {open&&(
        <div style={{padding:'12px 16px 16px'}}>
          <div style={{display:'flex',gap:8,marginBottom:12}}>
            {mode==='schedule'?(
              <>
                <div style={{flex:1,borderRadius:12,padding:'10px 12px',background:'rgba(52,211,153,0.05)',border:'1px solid rgba(52,211,153,0.1)'}}>
                ...
                {scheduleStatus?.nextExecutionTime&&(
                  <div style={{flex:1,borderRadius:12,padding:'10px 12px',background:'rgba(52,211,153,0.05)',border:'1px solid rgba(52,211,153,0.12)'}}>
                    <span style={{display:'block',fontSize:9,fontWeight:700,textTransform:'uppercase',letterSpacing:'0.1em',color:C.muted}}>Berikutnya</span>
                    <span style={{fontSize:15,fontWeight:700,lineHeight:'1.1',fontFamily:'monospace',color:'#6ee7b7'}}>{scheduleStatus.nextExecutionTime}</span>
                  </div>
                )}
              </>
            ):(
              <>
                <div style={{flex:1,borderRadius:12,padding:'10px 12px',background:'rgba(0,0,0,0.25)',border:`1px solid ${pnlPos?'rgba(52,211,153,0.12)':'rgba(248,113,113,0.12)'}`}}>
                  ...
                  <span style={{fontSize:17,fontWeight:700,lineHeight:'1.1',color:pnlPos?C.cyan:C.coral}}>{pnlPos?'+':''}{profit.toLocaleString('id-ID')}</span>
                </div>
                <div style={{flex:1,borderRadius:12,padding:'10px 12px',background:'rgba(52,211,153,0.05)',border:'1px solid rgba(52,211,153,0.1)'}}>
                ...
                {wr!==null&&<div style={{flex:1,borderRadius:12,padding:'10px 12px',background:'rgba(52,211,153,0.05)',border:'1px solid rgba(52,211,153,0.1)'}}>
                  <span style={{display:'block',fontSize:9,fontWeight:700,textTransform:'uppercase',letterSpacing:'0.1em',color:C.muted}}>Win%</span>
                  <span style={{fontSize:17,fontWeight:700,lineHeight:'1.1',color:wr>=50?C.cyan:C.coral}}>{wr}%</span>
                </div>}
              </>
            )}
          </div>
...
export default function DashboardPage() {
  ...
  const loadAll = useCallback(async(silent=false)=>{
    ...
    try{
      const [assRes,balRes,schRes,ordRes,logRes,ftRes,ftLogRes] = await Promise.allSettled([
  ...
  const blockedModes: TradingMode[] = (() => {
    const b: TradingMode[] = [];
    if(isSchedRunning||isSchedPaused){b.push('fastrade');b.push('ctc');}
    if(isFtRunning&&ftStatus?.mode==='FTT'){b.push('schedule');b.push('ctc');}
    if(isFtRunning&&ftStatus?.mode==='CTC'){b.push('schedule');b.push('fastrade');}
    return b.filter((v,i,a)=>a.indexOf(v)===i);
  })();

  const isActiveMode = tradingMode==='schedule'
    ? (isSchedRunning||isSchedPaused)
    : isFtRunning&&ftStatus?.mode===(tradingMode==='ctc'?'CTC':'FTT');
    ...
    <div style={{display:'grid',gridTemplateColumns:deviceType==='desktop'?'repeat(4,1fr)':deviceType==='tablet'?'repeat(3,1fr)':'1fr 1fr',gap:g}}>
      ...
      {deviceType==='desktop'&&(
        <Card style={{padding:'11px 14px'}}>
          ...
          <p style={{fontSize:18,fontWeight:700,color:isActiveMode?(tradingMode==='ctc'?C.violet:C.cyan):C.muted}}>{tradingMode==='ctc'?'CTC':tradingMode==='fastrade'?'FastTrade':'Signal'}</p>
          <StatusChip col={isActiveMode?(tradingMode==='ctc'?C.violet:C.cyan):C.muted} label={isActiveMode?'Running':'Standby'}/>
        </Card>
      )}
    </div>
  );
  ...
  const ModeSession = (fillH:boolean) => (
    <ModeSessionPanel
      ...
      ftStatus={ftStatus} ftLogs={ftLogs} ftLoading={false} fillHeight={fillH}
    />
  );
      ...
      {deviceType!=='desktop'&&(
        <div style={{position:'relative',overflow:'hidden',marginBottom:20}}>
          ...
          <div style={{padding:'12px 16px',display:'flex',alignItems:'center',justifyContent:'space-between',background:'rgba(30,92,60,0.3)',borderBottom:`1px solid ${C.bdr}`}}>
            <div style={{display:'flex',alignItems:'center',gap:10}}>
              <div style={{width:32,height:32,borderRadius:9,background:`${C.cyan}12`,border:`1px solid ${C.cyan}25`,display:'flex',alignItems:'center',justifyContent:'center',fontSize:16}}>📈</div>
              <div>
              ...
              <Card style={{padding:16,display:'flex',flexDirection:'column',minHeight:420}}>
                ...
                {selectedRic&&(
                  <div style={{display:'flex',alignItems:'center',justifyContent:'center',gap:5,marginTop:12}}>
                    <span style={{width:4,height:4,borderRadius:'50%',background:tradingMode==='ctc'?C.violet:C.cyan,opacity:0.6}}/>
                  ...
                  <div style={{flex:1,display:'flex',flexDirection:'column',justifyContent:'flex-end',gap:5,marginTop:8}}>
                    {selectedRic&&(
                      <div style={{display:'flex',alignItems:'center',justifyContent:'center',gap:5,padding:'3px 8px',borderRadius:6,background:C.faint}}>
                        <span style={{width:4,height:4,borderRadius:'50%',background:tradingMode==='ctc'?C.violet:C.cyan,opacity:0.6,flexShrink:0}}/>
                    ...
                    <div style={{display:'flex',justifyContent:'space-between',alignItems:'center'}}>
                      ...
                      <span style={{display:'flex',alignItems:'center',gap:4,fontSize:9,fontWeight:600,color:isActiveMode?(tradingMode==='ctc'?C.violet:C.cyan):C.muted}}>
                        <span style={{width:5,height:5,borderRadius:'50%',background:isActiveMode?(tradingMode==='ctc'?C.violet:C.cyan):'rgba(255,255,255,0.2)'}}/>
